...
class MatchingModel(nn.Module):
    ...
    def compile(self, lr=1e-3, optimizer=None, loss=None, **kwargs):
        ...
        elif loss == "MultiCosineContrastiveLoss":
            pos_weight = kwargs.get("pos_weight")
            neg_weight = kwargs.get("neg_weight")
            margin_values_list = kwargs.get("margin_values_list")
            negative_weights_list = kwargs.get("negative_weights_list")
            self.loss_fn = losses.MultiCosineContrastiveLoss(
              pos_weight=pos_weight,neg_weight=neg_weight,
              margin_values_list=margin_values_list, 
              negative_weights_list=negative_weights_list)
        elif loss == "BSLLoss":
            temperature = kwargs.get("temperature")
            temperature2 = kwargs.get("temperature2")
            mode = kwargs.get("mode")
            self.loss_fn = losses.BSLLoss(
